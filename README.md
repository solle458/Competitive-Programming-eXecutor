...
4. `atcoder_session` を [online-judge-tools](https://github.com/online-judge-tools/oj) の cookie.jar に同期
5. `oj submit <task-url> submission.{lang} --yes` を実行

**要件:**

- `oj` がインストール済み（`pip install online-judge-tools`）
- `atcoder_session` または `ATCODER_SESSION` が設定済み
- **開催中コンテストのみ** — 過去問はブラウザから提出
